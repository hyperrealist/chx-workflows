...
def get_sid_filenames(run):
    ...
    for resource_uid, resource in resources.items():
        ...
        for datum in datums[resource_uid]:
            ...
            filepaths.update(new_filepaths)
    return run.start["scan_id"], run.start["uid"], list(filepaths)
...
def get_frame_coords(coords, shape, frame_number):
    """
    Return the coords for the frame and flatten them into a 1D array.
    """
    offset = shape[-1] * shape[-2] * frame_number
    match_frame = np.where(coords[1] == frame_number)[0]
    coord_slice = coords[:,match_frame[0]:match_frame[-1]]
    return np.ravel_multi_index(coord_slice, sparse_images.shape) - offset


def get_frame_array(images, frame_number):
    return images[0][frame_number].data
...
def sparsify_improved(ref):
    ...
    def write_frame(images, frame_number):

        image_count = images.shape[1]
        image = images[0][frame_number]
